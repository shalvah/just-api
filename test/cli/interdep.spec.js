...
describe('Inter suite spec dependencies', function () {
    let suiteContext = this;

    before(async function () {
        let result = runJustAPIJSON('suitedependencies/intersuitedeps.suite.js');
    ...
    it('beforetest hook inter-dependent', function () {
    ...
    it('beforetest hook inter-dependent enabled', function () {
        let result = suiteContext.result;
        let test = result.tests.find(t =>  t.name === this.test.title);
        expect(test.status).to.equal('pass');
        expect(test.error).to.be.a('null');
    ...
    it('beforetest hook inter-dependent validate response - should fail', function () {
        let result = suiteContext.result;
        let test = result.tests.find(t =>  t.name === this.test.title);
    ...
    it('beforetest hook inter-dependent error in hook - should fail', function () {
        let result = suiteContext.result;
        let test = result.tests.find(t =>  t.name === this.test.title);
        expect(test.status).to.equal('fail');
        expect(test.error.name).to.equal('BeforeTestHookError');
    ...
    it('beforetest hook inter-dependent no matching spec - should fail', function () {
        let result = suiteContext.result;
        let test = result.tests.find(t =>  t.name === this.test.title);
        expect(test.status).to.equal('fail');
        expect(test.error.name).to.equal('BeforeTestHookError');
